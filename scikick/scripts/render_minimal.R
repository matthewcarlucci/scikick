# This script is typically executed via Snakefile > Rscript
# Inputs: 
# 	Markdown file location 
#	Output directory
#	Location of internal scikick templates
#	Location of projects index.html
# Output: HTML file
# Notes:
# Markdown file should have access to all refenreced content 
# A _site.yml file will be used by rmarkdown::render to generate
# the navbar of the website.

render_minimal <- function(input,outfile,templatedir,index_html){
    outdir = dirname(outfile)    

    message = sprintf("sk: Converting %s to site page %s", input, outfile)
    write(message, stderr())

    # Read the knit_meta() output generated by knit.R
    # which is a must for interactive plots (js libraries)
    knitmeta_filename = sub(input, pattern = ".md$",
        replacement = ".knitmeta.RDS")
    knitmeta = list()
    if(file.exists(knitmeta_filename)){
        knitmeta = readRDS(knitmeta_filename)
    }
    
    # Force pandoc to look in multiple locations for resources
    #project_root = getwd()
    #markdown_file_dir = file.path(getwd(), dirname(input))
    #pandoc_args = paste0("--resource-path=", project_root , ":",
    #        markdown_file_dir
    #        # Eventually adding the original script dir could be useful 
    #      )
         
    # Use of no extension allows for arbitrary outputs
    outfile_base = tools::file_path_sans_ext(outfile)
    # generate the html file
    rmarkdown::render(input,
        output_dir = outdir,
        output_file = outfile_base, 
        knit_meta = knitmeta
        )

    # Modify html to point to top level index.html file
    html_outfile = sub(pattern = ".md$", replacement = ".html",
        file.path(outdir, basename(input)))
    html_text = readLines(html_outfile)

    out = gsub(pattern="(navbar-brand.*)index.html",
        replacement=paste0("\\1", index_html),
            x=html_text)
    writeLines(text=out, con=html_outfile)
}

# Allows this file to be used with source() or Rscript
if(!interactive()){
    # loading snakefile variables
    args = commandArgs(trailingOnly=TRUE)
    input = args[1]
    outfile = args[2]
    templatedir = args[3]
    index_html = args[4]
    render_minimal(input,outfile,templatedir,index_html)
}
