...
test_datadir = os.path.join(exe_dir, "data", "test_conda")
code_dir = os.path.join(test_datadir, "code")
yaml_file = os.path.join(test_datadir, "scikick.yml")
simg_file = os.path.join(test_datadir, "env.yml")

htmls = ["analysis_normalization.html", "analysis_summary.html",
	"index.html", "test_python.html", "analysis_oscillations.html",
	"data_generate.html", "test_bash.html"]
...
class TestConda:
	...
	def setup(self):
		...
		shutil.copy2(simg_file, self.project_dir.name)
	...
	def test_singularity(self):
		assert os.system("scikick snake --conda env.yml") == 0
		assert os.system("scikick snake --args-set=\"--use-conda\"") == 0
